...
box_plot_list[[6]] <- clust %>% 
  ...
  scale_fill_gradient2(
    low = "#FFADAD", mid = "grey", high = "#a0c4ff", midpoint = 1,
    # trans = "sqrt",  # apply sqrt only to the scale, not the data
    # oob = scales::squish,
    # labels = function(x) round(x^2)  # invert sqrt by squaring
  ) + 
  geom_text(aes(label = paste0(number_of_studies, "\n", "(", round(research_share_to_pop_share, 2), ")")), 
            size = 2, lineheight = .8, col = "white") +
  scale_color_manual(values = c("black", "white")) +
  theme_SM() + 
