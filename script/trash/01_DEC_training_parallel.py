...
def run_single_experiment(args):
    run_id, n_clusters = args
    result = train_run(run_id, n_clusters, cities_clean_scaled)
    print(f"Finished run {run_id} for {n_clusters} clusters")
    return result

def run_experiments_parallel(cluster_range, n_runs, n_processes=None):
    all_args = [(run_id, n_clusters) for n_clusters in cluster_range for run_id in range(n_runs)]

    with mp.get_context("spawn").Pool(processes=n_processes or mp.cpu_count()) as pool:
        results = pool.map(run_single_experiment, all_args)
...
def build_autoencoder(hp):
    ...
    x = layers.Dense(units1, activation='relu', kernel_initializer=init, kernel_regularizer=reg)(inputs)
    x = layers.Dense(units2, activation='relu', kernel_initializer=init, kernel_regularizer=reg)(x)
    encoded = layers.Dense(encoding_dim, activation='relu', kernel_initializer=init, kernel_regularizer=reg, name='encoder_output')(x)

    x = layers.Dense(units2, activation='relu', kernel_initializer=init, kernel_regularizer=reg)(encoded)
    x = layers.Dense(units1, activation='relu', kernel_initializer=init, kernel_regularizer=reg)(x)
    decoded = layers.Dense(input_dim, activation='sigmoid', kernel_initializer=init, kernel_regularizer=reg)(x)
...
def train_autoencoder(run_id, model_dir='clustering_models/models'):
    ...
    tuner._save_trial = lambda trial: None
...
if __name__ == '__main__':
    import multiprocessing as mp
    mp.set_start_method('spawn', force=True)  # safe for TensorFlow

    cluster_range = range(3, 12)
    n_runs = 60
    n_processes = 30  # Set to desired number of parallel processes

    performance_scores = run_experiments_parallel(cluster_range, n_runs, n_processes)
