...
clust <- clust %>% 
  ...
  arrange(consensus_label_majority, -similarity) %>% 
  mutate(representative_city = row_number()<=5)
...
desc_dat_long <- desc_dat %>% 
  mutate(GHS_population = case_when(GHS_population <=.3 ~ "Small",
                                    GHS_population >.3 & GHS_population <=1 ~ "Medium",
                                    GHS_population >1 & GHS_population <=10 ~ "Large",
                                    GHS_population >10 ~ "Mega"),
         GHS_population_density = case_when(GHS_population_density <=0.01 ~ "Low",
                                            GHS_population_density >0.01 & GHS_population_density <=.03 ~ "Lower-middle",
                                            GHS_population_density >0.03 & GHS_population_density <=.05 ~ "Upper-middle",
                                            GHS_population_density >0.05 ~ "High"),
         GHS_population_growth = case_when(GHS_population_growth <=0 ~ "Decline",
                                           GHS_population_growth >0 & GHS_population_growth <=1 ~ "Slow",
                                           GHS_population_growth >1 & GHS_population_growth <=2 ~ "Moderate",
                                           GHS_population_growth >1 ~ "Fast"),
         GHS_population_density_growth = case_when(GHS_population_density_growth <=-1 ~ "Extreme sprawl",
         ...
         GHS_GDP_PPP = case_when(
           GHS_GDP_PPP <= 2 ~ "Low",                 # Mostly underdeveloped or poor areas
           GHS_GDP_PPP > 2 & GHS_GDP_PPP <= 6 ~ "Lower-middle",   # Developing
           GHS_GDP_PPP > 6 & GHS_GDP_PPP <= 15 ~ "Upper-middle",  # Emerging
           GHS_GDP_PPP > 15 ~ "High"                 # Affluent urban areas
         ),
         GHS_GDP_PPP_growth = case_when(
           GHS_GDP_PPP_growth <= 0 ~ "Decline",                       # Economic Decline
           GHS_GDP_PPP_growth > 0 & GHS_GDP_PPP_growth <= 2.5 ~ "Slow",   # Lower quartile
           GHS_GDP_PPP_growth > 2.5 & GHS_GDP_PPP_growth <= 6.7 ~ "Moderate", # Interquartile range
           GHS_GDP_PPP_growth > 6.7 ~ "Fast"                   # Top quartile and beyond
         ),
         # GHS_urban_sprawl = case_when(
         #   GHS_urban_sprawl <= 0 ~ "Low",
         #   GHS_urban_sprawl > 0 & GHS_urban_sprawl <= 1 ~ "Lower-middle",
         #   GHS_urban_sprawl > 1 & GHS_urban_sprawl <= 2.5 ~ "Upper-middle",
         #   GHS_urban_sprawl > 2.5 ~ "High",
         # ),
         ...
         hdd = case_when(
           hdd <= 0.03 ~ "Very mild",
           hdd > 0.03 & hdd <= 0.5 ~ "Mild",
           hdd > 0.5 & hdd <= 2 ~ "Cold",
           hdd > 2 ~ "Very cold"
         ),
         cdd = case_when(
           cdd <= 0.8 ~ "Very mild",
           cdd > 0.8 & cdd <= 1.8 ~ "Mild",
           cdd > 1.8 & cdd <= 3 ~ "Hot",
           cdd > 3 ~ "Very hot"
         )
         ) %>%
  pivot_longer(co_vars, names_to = "co_vars", values_to = "co_var_value") %>% 
  # pivot_longer(reg_vars, names_to = "reg_vars", values_to = "reg_vars_value") %>% 
  mutate(co_var_value = factor(co_var_value, levels = c(
    ...
    "Decline", "Slow", "Moderate", "Fast",
    
    "Compactification", "Some sprawl", "Much sprawl",  "Extreme sprawl",
    # "Low", "Lower-middle", "Upper-middle", "High",
    
    # "Decline", "Slow growth", "Moderate growth", "Fast growth",
    
    # "Low", "Lower-middle", "Upper-middle", "High",
    
    "Arid", "Semi-arid", "Temperate", "Humid",
    
    "Very mild", "Mild", "Cold", "Very cold",
    
    # "Very mild summer", "Mild summer", 
    "Hot", "Very hot"
  ))
  )
...
# NAMES
#####################
# 0) "Lower-Middle Income Urban Regions with Mixed Sprawl and Moderate Economic Growth in Warm Climates "

# This name reflects:
#   
#   “Transitional” – Lower-middle income status with moderate GDP growth and infrastructure pressures.
# “Urban” – High population density and sprawling development patterns.
# “Growth Zones” – Moderate economic and population growth with rising climate and energy demands (hot summers, mild winters).

# 1 ) "High-Income Temperate Urban Areas with Cold-Climate Heating and Emerging Cooling Needs"

# Adaptation challenges:
#   Cold climate heating demand dominates → energy infrastructure for heating, cold stress
# Temperate precipitation → moderate weather extremes but seasonal variation
# Compact urban growth → potential for efficient infrastructure but risk of overheating or energy demand peaks
# Mitigation challenges:
#   High-income, slow growth → high baseline emissions but potentially stable or slightly declining emissions
# Mixed cooling demand, with mild to very mild → growing cooling needs but less extreme than hotter regions
# Compactification can reduce emissions per capita, but requires managing heat and energy efficiency

# 2) "Declining Small Cities with Heating-Dominated Energy Use"

# 🔧 Interpretation
# Adaptation challenges:
#   Cold climate → heating demand and related energy infrastructure
# Temperate climate with moderate cooling needs
# Sprawl → inefficient infrastructure, higher exposure, costs
# Mitigation challenges:
#   Fast economic growth → rising emissions potential
# Sprawl + heating demand = high energy use, emissions
# Mixed cooling demand signals transitional energy profiles

# 3) "Sprawling Cold-Climate Regions with Moderate Economic Growth"

# Adaptation challenges:
#   Cold climates → heating needs
# Semi-arid → water stress, possibly drought-sensitive
# Sprawl → infrastructure cost, low adaptive efficiency
# Mitigation challenges:
#   Sprawl + cold climate = high heating-related emissions
# Energy demand structure skewed toward heating
# Upper-middle income & economic growth = rising consumption patterns
  
# 4) Fast-Growing Small to Medium Cities with Elevated Cooling Demand

# 🔧 Interpretation
# Adaptation:
#   Humid climates with high cooling needs — heat stress and energy demand issues
# Urban densification means managing infrastructure and urban heat island effects
# Mitigation:
#   Moderate economic growth, predominantly lower-middle income — potential for clean growth but constrained resources
# Urban compaction could reduce emissions per capita, but increasing cooling demand may offset gains


# 5) Large Low-Income Urban Regions with Hot Climates and Rapid Population Expansion

# Adaptation challenges:
#   High urban density with fast growth → infrastructure stress and urban heat risk
# Hot climates with strong cooling demand → energy and health vulnerability
# Temperate to semi-arid/arid climates → water stress and extreme weather exposure
# Mitigation challenges:
#   Low-income but fast-growing economies → emissions rising, limited resources for clean tech
# Compactification may help emissions intensity but urban heat and energy demand are big concerns

cluster_names <- data.frame(
  consensus_label_majority = 0:3,
  cluster_name = c(
    "Lower-Middle Income Cities with Moderate Economic Growth in Warm Climates",
    "Small to Medium Temperate Cities with Moderate Growth and High Cooling Demand",
    "Declining Small Cities with Heating-Dominated Energy Use",
    "Sprawling Cold-Climate Cities with Moderate Economic Growth"
    # "Fast-Growing Small to Medium Cities with Elevated Cooling Demand",
    # "Fast-growing Mega Cities with mostly Hot Climates"
  )) %>% 
  mutate(cluster_name = factor(cluster_name, levels = cluster_name)) %>% 
  mutate(
    cluster_name_wrapped = recode(
      cluster_name,
      "Lower-Middle Income Cities with Moderate Economic Growth in Warm Climates" =
        "Lower-Middle Income Cities\nwith Moderate Economic\nGrowth in Warm Climates",
      "Small to Medium Temperate Cities with Moderate Growth and High Cooling Demand" =
        "Small to Medium Temperate Cities\nwith Moderate Growth\nand High Cooling Demand",
      "Declining Small Cities with Heating-Dominated Energy Use" =
        "Declining Small Cities with\nHeating-Dominated\nEnergy Use",
      "Sprawling Cold-Climate Cities with Moderate Economic Growth" =
        "Sprawling Cold-Climate Cities\nwith Moderate\nEconomic Growth"
      # "Fast-Growing Small to Medium Cities with Elevated Cooling Demand" =
      #   "Fast-Growing Small to Medium\nCities with Elevated\nCooling Demand",
      # "Fast-growing Mega Cities with mostly Hot Climates" =
      #   "Fast-growing Mega Cities\nwith mostly Hot Climates"
    )
  )
...
for (cluster in min_cl:max_cl) {
  ...
  p_types_map <- desc_geo %>% 
    ...
    ggrepel::geom_label_repel(
      aes(label = ifelse(representative_city == "representative", GC_UCN_MAI_2025, NA), geometry = centroid),
      stat = "sf_coordinates", alpha=.5, size = 2, 
    ) +
  ...
  for (co_var in unique(desc_dat_long$co_vars)){
    p_heat[[co_var]] <- desc_dat_long %>%
      filter(consensus_label_majority == cluster,
             co_vars == co_var) %>%
      pivot_longer(
        cols = c(share_bin, share_clus, fill_val_bin, fill_val_clus, max_value_bin, max_value_clus),
  ...
  plot.with.inset <-
    ...
    draw_plot(p_heat_combined, x = 0, y = 0, width = 1, height = .42) + 
    theme(plot.margin = margin(c(15,5,0,5), "cm"))
  
  box_plot_list[[cluster+1]] <- plot.with.inset
}

multi_panel_plot <- plot_grid(plotlist = box_plot_list, ncol = 2, labels = "auto")
ggsave(multi_panel_plot, filename = paste0("plots/multi_panel_plot_40.pdf"), width = 10, height = 7)
...
p_regional_types <- ggplot() + 
  ...
  geom_sf_pattern(
    ...
    aes(pattern_fill = as.factor(consensus_label_majority), 
        pattern = as.factor(consensus_label_majority),
        pattern_angle = as.factor(consensus_label_majority)),
    pattern_density = .7,
...
ggsave(p_regional_types, filename = "plots/p_regional_types.pdf", width = 10, height = 6)
...
clean_places <- clean_places %>% 
  filter(!is.na(city_intersection_id) & !is.na(city_word_match_id)) %>% # move this data clenaing step to the previous script
...
p_cities_per_cluster_n <- clust %>%  
  ...
  ggplot(aes(x = cluster_name_wrapped, y = n_cities, fill = any_study_true)) +  
...
labelled_topics <- readxl::read_xlsx("validation/labelled_topics_1.xlsx")
main_topic <- read.csv("data/topic_model/main_topic_270.csv")
...
p_over_under_researched_by_clust <- left_join(n_case_studies_per_cluster, n_cities_per_cluster, by = "consensus_label_majority") %>% 
  ...
  mutate(ratio_type = ifelse(ratio_type == "ratio_with_cities", "Number of cities", "Number of studies")) %>% 
  left_join(cluster_names, by = "consensus_label_majority") %>% 
  ggplot(aes(x = cluster_name_wrapped, y = ratio, shape = ratio_type, col = ratio_type)) +  
  geom_point(size = 3) + 
...
growth_by_phase <- data_phase %>%
  filter(!is.na(phase)) %>%
  group_by(phase, cluster_name_wrapped, continent) %>%
  summarise(n_studies = n(), .groups = "drop") %>%
  mutate(phase = factor(phase, levels = c("AR1", "AR2", "AR3", "AR4", "AR5", "AR6"))) %>%
  arrange(cluster_name_wrapped, continent, phase) %>%
  # summarise(n_studies = sum(n_studies)) %>% 
  group_by(cluster_name_wrapped, continent) %>%
  mutate(
...
growth_by_phase_anno <- growth_by_phase %>% 
  group_by(cluster_name_wrapped, phase) %>% 
  summarise(n_studies = sum(n_studies)) %>% 
...
p_g_rel <- ggplot(growth_by_phase, aes(x = year, y = norm_growth, color = continent, group = continent)) +
  geom_line(size = 1) +
  geom_point(size = 1.2) +
  facet_wrap(~cluster_name_wrapped) +
  geom_text(
    data = annotations,
...
annotations <- data_phase %>%
  filter(!is.na(phase)) %>%
  group_by(phase, cluster_name_wrapped) %>%
  ...
  mutate(
    ...
    y = 2400  # start from 0; we’ll place it just above x-axis
  )

# Step 3: Main plot with bars, vlines, and annotations
p_g_abs <- data_phase %>%
  filter(publication_year >= 1990 & publication_year <= 2022) %>%
  group_by(publication_year, cluster_name_wrapped, continent) %>%
  summarise(n_studies = n(), .groups = "drop") %>%
...
co_mat <- cluster_cities %>%
  ...
  distinct() %>%
  inner_join(research_metrics %>% mutate(ID_UC_G0 = as.numeric(ID_UC_G0)), by = "ID_UC_G0")  # ensure alignment
...
co_mat <-  co_mat %>%
  mutate(
    similarity_decile = ntile(similarity_sums, 10)/10,  # ranks from 1 to 10
...
teach_potential_volume <- co_mat %>%
  ...
  theme(legend.title = element_text(),
        legend.position = "none", 
        axis.title = element_text(size = 10)) +
  labs(x = "Cluster", y = "Number of studies", fill = "has research", subtitle = "Teaching: similarity-weighted\nnumber of studies")
...
teach_potential_evenness <- co_mat %>%
  ...
  theme(legend.title = element_text(),
        legend.position = "none", 
        axis.title = element_text(size = 10)) +
  labs(x = "Cluster", y = "Diversity", fill = "has research", subtitle = "Teaching: diversity\nof the topics")
...
learn_potential_volume <- co_mat %>%
  ...
  theme(legend.title = element_text(),
        ...
        axis.title = element_text(size = 10)) +
  labs(x = "Cluster", y = "Number of studies", fill = "has research", subtitle = "Learning: similarity-weighted\nnumber of studies")
...
learn_potential_evenness <- co_mat %>%
  ...
  theme(legend.title = element_text(),
        legend.position = "none", 
        axis.title = element_text(size = 10)) +
  labs(x = "Cluster", y = "Gini score", subtitle = "Learning: diversity\nof the topics")
...
ghsl_df <- ghsl %>%
  st_transform(proj_robin) %>%
  mutate(geom = st_centroid(geom)) %>%
  left_join(learn_pot %>% dplyr::select(ID_UC_G0, teaching_volume_potential), by = "ID_UC_G0") %>%
  mutate(
    x = st_coordinates(geom)[, 1],
    y = st_coordinates(geom)[, 2]
  ) %>%
...
# Interpolate (e.g., IDW)
idw_result <- idw(formula = teaching_volume_potential ~ 1, locations = ghsl_df, newdata = grid_df)
...
is_not_within <- st_intersection(r_sf, bb)
is_not_within <- sapply(is_not_within, is_empty)
r_sf2 <- r_sf %>% filter(!is_not_within)

# Plot in ggplot
p_aggregate_teaching_potential <- ggplot() +
  geom_sf(data = world %>% st_union(), fill = "white", color = "black", size = .3, alpha = .2) +  # World map with light gray color
...
p_aggregate_teaching_potential
ggsave(p_aggregate_teaching_potential, file = "plots/p_aggregate_teaching_potential.pdf", width = 10, height = 6)
