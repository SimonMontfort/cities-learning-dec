...
for (cluster in min_cl:max_cl) {
  ...
  plot.with.inset <-
    ...
    draw_plot(p_heat_combined, x = 0, y = 0, width = 1, height = .46) + 
...
ggsave(multi_panel_plot, filename = paste0("plots/multi_panel_plot.pdf"), width = 10, height = 8)
...
ghsl_full <- st_transform(ghsl_full, proj_robin)

coast <- world %>%
  st_union() %>%
  st_as_sf()

# 2. Compute centroids of ghsl_full polygons
ghsl_full_cent <- ghsl_full %>%
  mutate(geom = st_centroid(geom))

# 3. Compute distance from each centroid to coastline
ghsl_full_cent <- ghsl_full_cent %>%
  mutate(dist_to_coast = st_distance(geom, coast)) 

# 4. Convert to numeric in kilometers (optional)
ghsl_full_cent$dist_to_coast_km <- as.numeric(ghsl_full_cent$dist_to_coast) / 1000

# 5. Create dummy for "coastal" (e.g., within 50 km)
coastal_cities <- ghsl_full %>%
  as.data.frame() %>% 
  mutate(is_coastal = ifelse(dist_to_coast_km <= 25, 1, 0)) %>% 
  select(is_coastal, ID_UC_G0)
  
ghsl %>% 
  left_join(coastal_cities, by = "ID_UC_G0") %>% 
  ggplot() + 
  geom_sf(aes(col = is_coastal))
