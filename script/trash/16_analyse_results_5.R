...
clust <- read.csv("data/clustering_results/dec_clusters_k5.csv")
...
cluster_names <- data.frame(
  consensus_label_majority = 0:4,
  cluster_name = c(
    "Sprawling, high density cities",
    "Compact sub-tropical cities",
    "Small cities with heating demand",
    "Fast-growing",
    "Mega cities"
  )) %>% 
...
ggsave(multi_panel_plot, filename = paste0("plots/multi_panel_plot.pdf"), width = 10, height = 10)
...
p_aggregate_teaching_potential <- ggplot() +
  geom_sf(data = world %>% st_union(), fill = "white", color = "black", size = .3, alpha = .2) +  # World map with light gray color
  geom_sf(data = r_sf, aes(fill = var1.pred, col = var1.pred), alpha = .9) +
  scale_fill_viridis_c() +
