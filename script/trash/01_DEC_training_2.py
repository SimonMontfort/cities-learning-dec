...
def get_embeddings(encoder_model, run_id, latent_dir='clustering_models/latent_representation'):
    ...
    embeddings = encoder_model.predict(cities_clean_scaled, batch_size=256)
...
def run_dec_clustering(encoder_model, n_clusters, initial_centers, run_id):
    ...
    for ite in range(maxiter):
        ...
        if ite % update_interval == 0:
            ...
            embeddings = encoder_model.predict(cities_clean_scaled, batch_size=256, verbose=0)
    ...
    embeddings = encoder_model.predict(cities_clean_scaled, batch_size=256, verbose=0)
    q_final = dec_model.predict(cities_clean_scaled, batch_size=256)
...
def run_experiments(cluster_range, n_runs, cities_clean_scaled):
    ...
    if gpu_active:
        ...
        num_cores = 30 #round(multiprocessing.cpu_count() * 0.4)
