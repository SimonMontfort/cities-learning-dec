...
cities_clean_scaled_Europe = cities_clean_scaled[cities_clean_scaled_df["continent"] == "Europe"]
...
def get_embeddings(encoder_model, cities_clean_scaled_Europe, run_id, latent_dir='clustering_models/latent_representation'):

    embeddings = encoder_model.predict(cities_clean_scaled_Europe, batch_size=1024)
...
def train_run(run_id, n_clusters, cities_clean_scaled_Europe):
    encoder_model = train_autoencoder(run_id)
    embeddings = get_embeddings(encoder_model, cities_clean_scaled_Europe, run_id)

    # Simple K-Means
    kmeans_labels, kmeans_centers, kmeans_scores = run_kmeans_clustering(cities_clean_scaled_Europe, n_clusters, run_id, seed)

    # Simple Hierarchical Clustering
    hierarchical_labels, hierarchical_scores = run_hierarchical_clustering(cities_clean_scaled_Europe, n_clusters)
...
def run_experiments(cluster_range, n_runs):
    ...
    for n_clusters in cluster_range:
        ...
        for run_id in range(n_runs):
            result = train_run(run_id, n_clusters, cities_clean_scaled_Europe)
