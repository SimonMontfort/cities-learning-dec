...
def build_autoencoder_fixed(encoding_dim=3, l2_reg=0.001, units1=64, units2=24):
    ...
    Use your best hyperparameters from previous runs here.
    """
    input_dim = cities_clean_scaled.shape[1]
    reg = l2(l2_reg)
...
def train_autoencoder(run_id, model_dir='clustering_models/models', use_tuning=False):
    ...
    if use_tuning:
        # Original hyperparameter search (slow)
        tuner = kt.Hyperband(
            ...
            overwrite=True  # Don't reload previous runs
        )
    ...
    # Optional: save model
    # model_path = os.path.join(model_dir, f'model_run_{run_id}')
    # encoder_model.save(model_path, save_format='tf')
...
def load_or_compute_embeddings(encoder_model, run_id, latent_dir='clustering_models/latent_representation'):
    """Load existing embeddings if available, otherwise compute them."""
    latent_path = os.path.join(latent_dir, f'latent_run_{run_id}.pkl')
    ...
    if os.path.exists(latent_path):
        ...
        print(f"Computing new embeddings for run {run_id}")
        return get_embeddings(encoder_model, run_id, latent_dir)
...
def run_dec_clustering(encoder_model, n_clusters, initial_centers, run_id):
    ...
    final_labels = q_final.argmax(axis=1).copy()
...
def write_result_to_csv(result, filepath):
    """Append single result to CSV immediately"""
    rows = []
    for method in ['kmeans simple', 'hierarchical simple', 'kmeans embedded', 'dec']:
        labels_str = ','.join(map(str, result[method]['labels']))
        row = f"{result['run_id']},{result['n_clusters']},{method},"
        row += f"{result[method]['scores']['silhouette']},"
        row += f"{result[method]['scores']['calinski']},"
        row += f"{result[method]['scores']['davies']},\"{labels_str}\"\n"
        rows.append(row)
...
def train_run(run_id, n_clusters, cities_clean_scaled, encoder_model, embeddings):
    # Clear session at start
    K.clear_session()
    gc.collect()

    if encoder_model is None:
        encoder_model = train_autoencoder(run_id, use_tuning=True)
    ...
    # Convert to lists to avoid memory issues
    result = {
        'run_id': run_id,
        ...
        'kmeans embedded': {
            'labels': kmeans_emb_labels.tolist(),
            'scores': kmeans_emb_scores
        },
        'dec': {
            'labels': dec_labels.tolist(),
            'scores': dec_scores
        }
    }
...
def run_experiments(cluster_range, n_runs, skip_if_exists=False):
    # Initialize CSV file with header if it doesn't exist
    results_file = "data/clustering_results/raw_clustering_scores.csv"
    if not os.path.exists(results_file):
        with open(results_file, 'w') as f:
            f.write("run_id,n_clusters,method,silhouette,calinski,davies,labels\n")

    # OUTER LOOP: Loop over each run_id
    for run_id in range(n_runs):
        print(f"\n===== Processing run_id {run_id} =====")

        # SKIP CHECK 1: Check if ALL clusters for this run_id are already done
        if skip_if_exists and os.path.exists(results_file):
            existing_df = pd.read_csv(results_file)
            completed_clusters = existing_df[existing_df['run_id'] == run_id]['n_clusters'].unique()
            # If we have results for all clusters in cluster_range for this run_id, skip entirely
            if len(completed_clusters) == len(cluster_range) and all(c in completed_clusters for c in cluster_range):
                print(f"Skipping run_id={run_id} (all clusters already completed)")
                continue

        # Train autoencoder and get embeddings ONCE per run_id
        print(f"Training autoencoder for run_id {run_id}")
        encoder_model = train_autoencoder(run_id)

        # Get or load embeddings
        embeddings = load_or_compute_embeddings(encoder_model, run_id)

        # INNER LOOP: Now cluster with different n_clusters using the SAME embeddings
        for n_clusters in cluster_range:
            # SKIP CHECK 2: Check if this specific (run_id, n_clusters) combination exists
            if skip_if_exists and os.path.exists(results_file):
                existing_df = pd.read_csv(results_file)
                if len(existing_df[(existing_df['run_id'] == run_id) & (existing_df['n_clusters'] == n_clusters)]) > 0:
                    print(f"  Skipping n_clusters={n_clusters} (already exists)")
                    continue

            print(f"  Clustering with {n_clusters} clusters")
            # Pass the encoder_model and embeddings so we don't retrain/recompute
            result = train_run(run_id, n_clusters, cities_clean_scaled, encoder_model, embeddings)

            # Write immediately to disk (append mode)
            write_result_to_csv(result, results_file)

            print(f"  Completed: KMeans embedded silhouette: {result['kmeans embedded']['scores']['silhouette']:.4f}, "
                  f"DEC silhouette: {result['dec']['scores']['silhouette']:.4f}")

            # Free memory immediately
            del result
            gc.collect()

        # After processing all n_clusters for this run_id, clean up the encoder and embeddings
        del encoder_model
        del embeddings
        K.clear_session()
        gc.collect()
...
if __name__ == '__main__':
    ...
    # Set skip_if_exists=True to resume interrupted runs
    run_experiments(cluster_range, n_runs, skip_if_exists=True)
