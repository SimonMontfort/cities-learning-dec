...
# ── 1. Strategic document types ───────────────────────────────────────────────
# Directional, non-binding executive instruments.
# NDCs, LEDS, Adaptation Communications etc. are excluded upstream via the
# Category filter (they are classified as UNFCCC in CCLW). NBSAP is the one
# exception that survives as Executive — added here explicitly.
strategic_types <- c(
  "Strategy", "Plan", "Action Plan", "National Adaptation Plan",
  "Programme", "Roadmap", "Policy", "Agenda", "Vision",
...
country_gov <- cclw_clean |>
  ...
  summarise(
    ...
    n_laws_decrees = sum(!is_strategic),
    .groups = "drop"
  )
...
stopifnot(
  ...
  all(country_gov$n_strategic   + country_gov$n_laws_decrees == country_gov$n_total)
)
...
# Variable labels for facets — order controls left-to-right display
gov_vars <- c(
  "n_total"        = "Total",
...
p_box <- ggplot(ucdb_long,
  ...
  labs(
    ...
    caption  = paste0(
      ...
      "reflects the political system outside consolidated democracies.\n",
      "Strategic = Strategy, Plan, Action Plan, Programme, Roadmap, Policy, Agenda, Vision, Decision And Plan, Law And Plan, NBSAP.\n",
      "Laws & decrees = Legislative + non-strategic Executive instruments. "
...
ggsave("plots/governance_by_type.pdf", p_box,
       width = 10, height = 5)

cat("Saved: figures/governance_by_type.png\n")